// NOTE: I ran the tests in this file using 1 worker, since making requests to
// HackerNews too fast causes the website to be overloaded.
// npx playwright test --workers=1
const { test, expect } = require('@playwright/test');
const { AxeBuilder } = require('@axe-core/playwright'); // 1
const classifyArticleTitles = require('../classify.js');
const { analyzePage, loginToHackerNews, analyzePageTest } = require('../page-analysis.js');
...
test.describe('end-to-end testing', () => {
  test('article title classifications', async ({ page }) => {
    ...
    expect(results).toStrictEqual(['not-interesting', 'interesting', 
      'not-interesting', 'interesting', 'interesting']);
  });

  // Implementation for testing analyze page, it's a bit buggy when running
  // in parallel
  // test('analyze page', async({ page }) => {
  //   const numInterestingArticles = await analyzePageTest(page);
  //   expect(numInterestingArticles).toBeGreaterThan(10);
  // });
});
